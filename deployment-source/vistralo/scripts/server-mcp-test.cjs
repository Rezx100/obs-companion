...
(async()=>{
  ...
  try{
    ...
    await client.connect(new StdioClientTransport({command:process.execPath,args:['src/server-mcp.cjs'],env:{...process.env,VISTRALO_INTERNAL_ORIGIN:origin,VISTRALO_TOKEN:token},stderr:'pipe'}));
