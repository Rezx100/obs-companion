...
(async()=>{
  ...
  const exe=process.env.VISTRALO_TEST_CHROMIUM||path.resolve('.cache/chromium/chromium');
  if(!process.env.VISTRALO_TEST_CHROMIUM){fs.mkdirSync(path.dirname(exe),{recursive:true});fs.writeFileSync(exe,require('node:zlib').brotliDecompressSync(fs.readFileSync('node_modules/@sparticuz/chromium/bin/chromium.br')));fs.chmodSync(exe,0o755);}
