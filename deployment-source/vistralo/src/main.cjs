...
const {Service}=require('./service.cjs');const {Vault,Providers}=require('./providers.cjs');const {assert,within,atomic}=require('./core.cjs');const brand=require('./brand.cjs');const {selectRoots}=require('./identity-paths.cjs');const {resolved}=require('./runtime-config.cjs');
protocol.registerSchemesAsPrivileged([{scheme:brand.scheme,privileges:{standard:true,secure:true,supportFetchAPI:true,stream:true}}]);
if(!process.argv.includes('--mcp')&&!app.requestSingleInstanceLock())app.quit();
let service,win,quitting=false;
app.whenReady().then(async()=>{
 const override=resolved(process.env,'VISTRALO_TEST_ROOT','OBS_COMPANION_TEST_ROOT');if(override!==undefined)assert(typeof override==='string'&&path.isAbsolute(override),'VISTRALO_TEST_ROOT must be an absolute directory');
 const roots=selectRoots({appData:app.getPath('appData'),documents:app.getPath('documents'),override});
 fs.mkdirSync(roots.userData,{recursive:true,mode:0o700});app.setPath('userData',roots.userData);const root=roots.projects;
 ...
 const methods={
  ...
  export:async a=>{const selected=await dialog.showOpenDialog(win,{title:'Choose parent folder for project export',properties:['openDirectory','createDirectory']});if(selected.canceled)return null;return service.exportProject(a.id,path.join(selected.filePaths[0],'Vistralo-Project-'+a.id+'-'+Date.now()));},
 ...
 ipcMain.handle('vistralo',async(event,{method,args={}})=>{assert(event.sender===win.webContents&&event.senderFrame===win.webContents.mainFrame&&event.senderFrame.url.startsWith('vistralo://app/'),'Unauthorized caller');assert(Object.hasOwn(methods,method),'Unsupported operation');assert(Buffer.byteLength(JSON.stringify(args))<2e6,'Request too large');try{return {ok:true,value:await methods[method](args)};}catch(e){return {ok:false,error:e.message};}});
 win=new BrowserWindow({width:1340,height:900,minWidth:980,minHeight:700,title:brand.name,backgroundColor:'#f4f4ef',webPreferences:{preload:path.join(__dirname,'preload.cjs'),contextIsolation:true,nodeIntegration:false,sandbox:true,webSecurity:true}});
 win.webContents.setWindowOpenHandler(()=>({action:'deny'}));win.webContents.on('will-navigate',(e,url)=>{if(!url.startsWith('vistralo://app/'))e.preventDefault();});win.webContents.session.setPermissionRequestHandler((wc,p,callback)=>callback(false));await win.loadURL('vistralo://app/index.html');
