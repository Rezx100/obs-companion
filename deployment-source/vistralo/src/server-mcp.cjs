...
const {resolved,config}=require('./runtime-config.cjs');
const origin=resolved(process.env,'VISTRALO_INTERNAL_ORIGIN','COMPANION_INTERNAL_ORIGIN','http://127.0.0.1:8787');
const runtime=config();
assert(new URL(origin).origin===origin&&['127.0.0.1','localhost'].includes(new URL(origin).hostname),'MCP bridge connects only to the same server');
let cookie,loginPromise;
async function login(){
  const response=await fetch(origin+'/login',{method:'POST',headers:{Origin:origin,'Content-Type':'application/json'},body:JSON.stringify({token:runtime.token})});
...
async function handle({method,params={}}){
  if(method==='initialize')return {protocolVersion:'2024-11-05',capabilities:{tools:{listChanged:false}},serverInfo:{name:require('./brand.cjs').slug+'-server',version:'0.2.0-preview'}};
