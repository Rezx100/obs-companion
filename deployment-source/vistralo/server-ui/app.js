...
action('upload',async e=>{
  ...
  try{
    ...
    const digest=await fileHash(file),suffix=id+':'+kind+':'+digest,key='vistralo-upload:'+suffix,legacyKey='obs-upload:'+suffix;
    let upload,stored=localStorage.getItem(key),legacy=localStorage.getItem(legacyKey);
    if(!stored&&legacy)stored=legacy;
    if(stored){try{const candidate=await rpc('uploadStatus',{upload:stored});if(candidate.project!==id||candidate.kind!==kind||candidate.digest!==digest||candidate.size!==file.size||candidate.offset>file.size||!['Uploading','Complete'].includes(candidate.state))throw Error('Checkpoint does not match source');upload=candidate;localStorage.setItem(key,stored);localStorage.removeItem(legacyKey);}catch{localStorage.removeItem(key);localStorage.removeItem(legacyKey);}}
    ...
    const result=await rpc('uploadComplete',{upload:upload.id});localStorage.removeItem(key);localStorage.removeItem(legacyKey);
